...
public class GLPixelFunctionsCached implements IGLPixelFunctions {

	private final	IGLPixelFunctions	pixelFunctions;
	private			IGLBindingSource	bindingSource;

	public GLPixelFunctionsCached(IGLPixelFunctions pixelFunctions) {
	...
	public void pixelStore(int pixelParameter, int value) {
		...
		if (parameter == GLPixelParameter.INVALID) {
			throw new IllegalArgumentException("Invalid pixelParameter.");
		}

		if (bindingSource.getPixelStorageMode(parameter) == value) {
			return;
		}
