...
public interface IGLTextureStore  {
	...
	GLInternalFormat	getInternalFormat		(int mipLevel);
	IExtent3D			getExtent				(int mipLevel);
	boolean				isMultisampled			();
	boolean				isImmutable				();
	int					getImmutableMipLevels	();
	int					getMaxDimension			(int mipLevel);
	int					getPixels				(int mipLevel);
}
