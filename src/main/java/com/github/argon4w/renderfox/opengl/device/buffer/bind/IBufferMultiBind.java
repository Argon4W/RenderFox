...
public interface IBufferMultiBind {

	IGLBufferBindInfo.AbstractBuilder builder(GLBufferBlockType bufferBlockType);
	...
	void bindBuffersRange(
			GLBufferBlockType	bufferBlockType,
			int					bufferBlockFirstIndex,
			IGLBufferBase	[]	bufferHandles,
			long			[]	bufferOffsets,
			long			[]	bufferLengths
	);

	void bindBuffersRange(
			GLBufferBlockType	bufferBlockTypes,
			int					bufferBlockFirstIndex,
			GLBufferBind[]		bufferBinds
	);

	void bindBuffersRange(IGLBufferBindInfo info);
}
